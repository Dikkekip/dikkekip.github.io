---
layout: post
title: "AutoCode: I Have the Credits. I Don’t Always Have the Time."
date: 2026-09-07 20:00:00 +0200
description: "Experimenting with autonomous coding agents, OpenClaw, multiple AI models, and a framework that keeps building and maintaining software when I don’t have time to prompt it myself."
...
[GPT-6 Astra is out](https://openai.com/index/gpt-6-astra/), and despite the rocky launch anxiously refreshing my codex for the model to drop, since its been avalibe I’ve been coding whenever I’ve had a little extra time available. And honestly, in my own projects, the improvement is real. Tasks run for longer, more prompts deliver something genuinely useful in one shot, frontend UI has finally taken a step forward, and computer use feels much faster.
...
The compute commitments are enormous. The [infrastructure spending plans](https://openai.com/index/announcing-the-stargate-project/) are enormous. At some point, the revenue has to justify it, which means businesses—and, to some extent, people like us who build and maintain technology—have to find enough valuable things to do with all that compute.

Token-based billing makes this particularly interesting. Once you start running agents continuously, “let it have another go” becomes an architectural and financial decision. How many passes does this task actually need? Which model should do them? Did the additional inference make the result better, or did we just spend more money arriving at roughly the same answer?
For companies to actually measure what they get out of the AI usage what is the ROI and how should we limit or specify the amount of to base billing that we are allowing our employees to get as an additional benefit?
...
The code is at [Dikkekip/autocode](https://github.com/Dikkekip/autocode). The [getting-started guide](https://github.com/Dikkekip/autocode/blob/master/docs/getting-started.md) and [native reference](https://github.com/Dikkekip/autocode/blob/master/docs/native-autonomy.md) cover the longer version. This is the outline, because there are a few more moving parts than cloning a repo and telling an agent to get on with it.
...
- A unique `boardId`, the application repository, and its base branch.

- Your personas: goals, `allowedPaths`, success observations, weights, and any custom `ideationPrompt` instructions.

- Registered research, planner, coder, and reviewer agent IDs, with the model you want each to use. The reviewer must be different from the coder.

- `quality.skillPath`, pointing to the readable `skills/prompt-engineering-expert/SKILL.md`, and verification commands that actually exercise your application.

- Deployment and independent workflow-check commands. Keep `workerConcurrency` at one while you find out whether your setup works.
...
#[AutoCode](https://github.com/Dikkekip/AutoCode),
